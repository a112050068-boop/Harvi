...
const VideoPreview: React.FC<{ project: VideoProject; opacity?: string }> = ({ project, opacity = 'opacity-40' }) => {
  ...
  const endSeconds = project.previewEnd;
      ...
      playerRef.current = new window.YT.Player(playerElement, {
        ...
        events: {
          onReady: (event: any) => {
            ...
            if (iframe) {
              iframe.classList.add('w-full', 'h-full', 'object-cover', 'scale-[1.5]', 'transition-opacity', 'duration-1000', opacity);
  ...
  }, [videoId, startSeconds, endSeconds, opacity]);
...
const VerticalBlock: React.FC<{ project: VideoProject }> = ({ project }) => (
  ...
  >
    <div className="relative aspect-[9/16] bg-[#080808] rounded-sm overflow-hidden border border-white/5 transition-colors duration-700 group-hover:border-blue-500/40">
      <img 
        ...
        <div className="space-y-12 md:space-y-40">
          {horizontalProjects.map((p, i) => (
            <div key={p.id} className="group px-3 md:px-0">
              ...
              >
                <div className="relative aspect-video bg-[#050505] overflow-hidden border-y border-white/5 transition-colors duration-1000 group-hover:border-blue-500/30">
                  <img 
                    src={p.thumbnail} 
                    alt={p.title} 
